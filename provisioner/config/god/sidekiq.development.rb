...
puts "sidekiq for god. env=#{app_env}"
...
God.watch do |w|
  ...
  w.group         = 'sidekiq'
  w.env           = { 'RAILS_ENV' => app_env, 'app_env'=>app_env, 'QUEUE' => '*' }
  w.dir           = app_root
  ...
  #
  sidekiq_options = "-e #{app_env} -r #{run_file} -t #{stop_timeout}  -c #{sidekiq_concurrency} -q #{sidekiq_queue} -L #{w.log} -P #{w.pid_file}"
  # -d
